  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none" aria-hidden>
        <svg className="absolute inset-0 w-full h-full block" viewBox="0 0 1080 1920" preserveAspectRatio="xMidYMid slice" fill="none">
          <defs>
            <filter id="glow" x="-40%" y="-40%" width="180%" height="180%"><feGaussianBlur stdDeviation="8"/></filter>
            <linearGradient id="lineOrange" gradientUnits="userSpaceOnUse" x1="-600" y1="560" x2="520" y2="760">
              <stop offset="0" stopColor="rgba(255,165,0,0)"/><stop offset=".55" stopColor="rgba(255,165,0,0.95)"/><stop offset="1" stopColor="rgba(255,165,0,0)"/>
            </linearGradient>
            <linearGradient id="lineCyan" gradientUnits="userSpaceOnUse" x1="1680" y1="560" x2="560" y2="760">
              <stop offset="0" stopColor="rgba(0,212,255,0)"/><stop offset=".55" stopColor="rgba(0,212,255,0.95)"/><stop offset="1" stopColor="rgba(0,212,255,0)"/>
            </linearGradient>
            <linearGradient id="stem" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0" stopColor="#66E0FF" stopOpacity=".45"/><stop offset="1" stopColor="#66E0FF" stopOpacity="0"/>
            </linearGradient>
          </defs>

          {/* left curve */}
          <path d="M -600 560 H 120 Q 260 560 360 640 L 520 740" stroke="url(#lineOrange)" strokeWidth="22" filter="url(#glow)" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
          <path d="M -600 560 H 120 Q 260 560 360 640 L 520 740" stroke="url(#lineOrange)" strokeWidth="9" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
          <path d="M -600 560 H 120 Q 260 560 360 640 L 520 740" stroke="white" strokeOpacity=".08" strokeWidth="2" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>

          {/* right curve */}
          <path d="M 1680 560 H 960 Q 820 560 720 640 L 560 740" stroke="url(#lineCyan)" strokeWidth="22" filter="url(#glow)" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
          <path d="M 1680 560 H 960 Q 820 560 720 640 L 560 740" stroke="url(#lineCyan)" strokeWidth="9" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
          <path d="M 1680 560 H 960 Q 820 560 720 640 L 560 740" stroke="white" strokeOpacity=".08" strokeWidth="2" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
      ...
      {/* ==== BEAMS ==== */}
      <div className="absolute inset-0 opacity-80 mix-blend-screen will-change-transform pointer-events-none" style={{ transform: tr(6) }} aria-hidden>
        <div className="absolute -inset-1 bg-[length:200%_200%]"
             style={{
               backgroundImage:
      ...
      {/* ==== HEX CELLS ==== */}
      <div className="absolute inset-0 pointer-events-none opacity-75 will-change-transform" style={{ transform: tr(6) }} aria-hidden>
        <svg viewBox="0 0 1080 1920" className="w-full h-full">
          <defs><linearGradient id="hx" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stopColor="#00e0ff"/><stop offset="1" stopColor="#00bfff"/></linearGradient></defs>
          {hexes.map((h, idx) => (
            <polygon key={idx} points={h.points} fill="transparent" stroke="url(#hx)" strokeWidth="1.2" className="[stroke-dasharray:10]" opacity={0.95}>
              <animateTransform attributeName="transform" type="translate" dur={`${h.dur/1.5}s`} begin={`${h.delay}s`} repeatCount="indefinite" values={`0 0; ${h.ax} ${h.ay}; 0 0`} keyTimes="0; 0.5; 1"/>
            </polygon>
          ))}
        </svg>
      </div>
      ...
      {/* ==== NETWORK ==== */}
      <div className="absolute inset-0 pointer-events-none opacity-70 will-change-transform" style={{ transform: tr(7) }} aria-hidden>
      ...
      {/* ==== PARTICLES ==== */}
      <div className="absolute inset-0 will-change-transform pointer-events-none" style={{ transform: tr(5) }} aria-hidden>
        {particles.map((p,i)=>(
          <span key={i} className="absolute block rounded-full bg-cyan-300 shadow-[0_0_10px_rgba(56,189,248,.9)] animate-[twkTwinkle_3.2s_ease-in-out_infinite]"
                style={{ width:`${p.w+1.5}px`, height:`${p.h+1.5}px`, left:`${p.l}%`, top:`${p.t}%`, animationDelay:`${p.d}s` }}/>
        ))}
      </div>
      ...
      <div className="absolute inset-0 z-[60]">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 top-[10vh] sm:top-[9vh] md:top-[8vh] pointer-events-none">
          <img src={FG_LOGO_SRC} alt={FG_LOGO_ALT} className="select-none"
               style={{ height: "12vh", width: "auto", filter: "drop-shadow(0 6px 24px rgba(0,0,0,.35))" }}/>
        </div>

        {/* Buttons — portrait дунд */}
        <div className="btn-belt pointer-events-auto z-[80]"
             style={{ position:"absolute", left:"50%", transform:"translateX(-50%)", top:"56vh", width:"min(92vw,1050px)", isolation:"isolate" }}>
      ...
      <style jsx>{`
        ...
        @media (max-aspect-ratio: 9/16){ .btn-belt{ top:58vh !important } }
        @media (max-aspect-ratio: 9/18){ .btn-belt{ top:60vh !important } }
        @media (min-aspect-ratio: 16/10){ .btn-belt{ top:64vh !important; width:min(88vw,1200px) !important } }
      `}</style>
    </div>
  );
}
...
function hexPoints(cx:number,cy:number,r:number){
  const pts:string[]=[]; for(let i=0;i<6;i++){ const a=(Math.PI/3)*i+Math.PI/6; pts.push(`${round3(cx+r*Math.cos(a))},${round3(cy+r*Math.sin(a))}`); }
  return pts.join(" ");
}
